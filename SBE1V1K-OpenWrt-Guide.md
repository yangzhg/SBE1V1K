...
更新日期：2026-08-26。设备别名包括 Spectrum SBE1V1K、Askey SBE1V1K、Askey RTQ7300T。

## 1. 先说结论

SBE1V1K 设备定义已经进入 OpenWrt 官方 `main`，但多 radio 稳定性和本项目附加
修复仍需真机验证：

- [PR #21586](https://github.com/openwrt/openwrt/pull/21586) 已于 2026-08-23
  合入；本项目固定在官方 `main` 提交 `6c12b87ffc`，直接采用正式设备定义。
- 设备提交包含 CPU、2 GB 内存、8 GB eMMC、四个以太网口、三频 Wi-Fi、LED、
  复位键和 PWM 风扇定义；原作者分支提交 `525e6238...` 仅保留为历史来源。
- QCN9274 BDF 已进入官方 `ipq-wifi` 源码。本分支继续携带每 radio MAC、稳定
  radio 映射和诊断补丁，因为 [Issue #24370](https://github.com/openwrt/openwrt/issues/24370)
  仍未在官方 `main` 中解决。
- 目前没有 IPQ9574 PPE/NSS 硬件路由加速支持；高吞吐转发按软件路径理解。
  设备定义合入不等于本项目附加修复已经完成真机验证，也不等于具备硬件卸载。
...
提交。`sources.lock` 记录官方基线、正式合入提交、历史设备来源、feeds 和外部
固件来源。
...
1. `qualcommbe: add support for Askey SBE1V1K`
   - 来源：[OpenWrt PR #21586](https://github.com/openwrt/openwrt/pull/21586)
   - 上游提交：`31c49dd07660a8c9a3b8a34f6fb66ec37244c948`
   - 状态：已合入 OpenWrt 官方 `main`，本仓库直接采用官方版本。
2. `wifi: ath12k: set per-radio MAC address from DT`
...
3. `2c64257627 wifi-scripts: support multiple device paths per board.json wlan entry`
...
4. `ipq-wifi: vendor Askey SBE1V1K BDF`
   ...
   - 状态：已由官方 `ipq-wifi` 源码包提供，本仓库不再内嵌二进制 BDF。
   - 用途：让三频 Wi-Fi 固件使用正确的板级数据。
...
本节到第 11 节描述的是 OpenWrt 官方安装路径。若选择
[YYH2913/http-uboot](https://github.com/YYH2913/http-uboot/tree/sbe1v1k)
的 HTTP 二阶段 chainloader，请改按 `SBE1V1K-UBOOT.md` 操作，不要把两套
环境变量、GPT 迁移和写入步骤混用。
...
- 设备支持和 BDF 已进入官方源码，但本项目的无线稳定性及平台补丁仍是本地
  差异；更新时必须逐项检查上游是否已有等价修复，再删除对应补丁。
- 尚无 PPE/NSS 硬件路由卸载，不能把端口线速等同于 NAT/防火墙线速。
- 2026 年 7 月出现的 HTTP chainloader/重分区方案不是 OpenWrt 官方安装路径；
  本仓库已在 `SBE1V1K-UBOOT.md` 单独审计。使用它前必须理解
  `mainline`/`large` 布局、备份边界和非原子写入风险。
...
本仓库已经迁移到包含正式设备支持的 OpenWrt `main`。后续更新继续以官方提交为
基线，只重放尚未上游化且经过构建、真机验证的差异。
