...
> 这是第三方、破坏性较强的可选方案，不是 OpenWrt 官方安装流程。只想保持官方
> `mainline` 分区时，可以完全不安装它，继续使用 initramfs + `fw_setenv` +
> `sysupgrade` 路径。
...
- `large`：创建 4 MiB `chainloader`、32 MiB `kernel`、1 GiB `rootfs` 和约 6.2 GiB `rootfs_data`；这是第三方重分区布局，不属于 OpenWrt 官方设备布局。
...
不要在这个流程中再执行官方安装路径的三条 `fw_setenv` 命令；chainloader 的
布局迁移会管理它需要的 `0:APPSBLENV`。两种安装流程应二选一，不能在中途混用。
...
|---|---:|---:|---|
| OpenWrt 官方路径：initramfs + `fw_setenv` + `sysupgrade` | 否 | 否 | 最接近上游、最少改动 |
